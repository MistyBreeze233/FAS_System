...
class FaceDataset(Dataset):
    def __init__(self, dataset_name, root_dir, split='train', label=None, transform=None, scale_up=1.1, scale_down=1.0,
                 map_size=32, UUID=-1):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name in DEVICE_INFOS:
            ...
            if device_tag is None:
                # 添加默认处理逻辑
                print(f"Warning: No match found for video {video_name}, using default device_tag.")
                device_tag = 'unknown'
        else:
